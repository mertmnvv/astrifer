  ...
  return (
    <NightPageShell widthPx={widthPx} heightPx={heightPx} printReady={true}>
      <div className="flex h-full w-full flex-col items-center justify-center px-[9%] py-[9%] text-ink">
        <p className="mb-2.5 text-center font-mono text-[8.5px] uppercase tracking-[0.24em]" style={{ color: theme.accentMetal }}>
          Yıldız Anahtarı
        </p>
        <div className="grid w-full grid-cols-2 gap-x-3.5 gap-y-0.5 font-mono text-[9px] leading-[1.9]" style={{ color: theme.text.body }}>
          {numberedStars.map(({ code, star }) => (
            <div key={code}>
              <span className="mr-1.5 font-bold" style={{ color: theme.accentMetal }}>
                {code}
              </span>
              {star.name}
            </div>
        ...
        <div className="my-3 h-px w-full" style={{ backgroundColor: `${theme.accentMetalDim}80` }} />
        <p className="text-center font-display text-[11px] italic leading-relaxed" style={{ color: theme.text.bodyMuted }}>
