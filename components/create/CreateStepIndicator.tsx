 ...
 * using the V2 archive/brass numbering language
            ...
            <li key={step.n} className="flex flex-1 items-center gap-1.5 sm:gap-2 last:flex-none">
              <button
                ...
                className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full border font-mono text-[10px] transition-colors ${
                  isCurrent
                    ? "border-[#c79a52] bg-[#c79a52]/10 text-[#c79a52]"
                    : isDone
                      ? "border-[#c79a52]/50 text-[#c79a52]/80 hover:border-[#c79a52]"
              ...
              {index < steps.length - 1 && <div className={`h-px flex-1 ${isDone ? "bg-[#c79a52]/40" : "bg-text/10"}`} />}
            </li>
          );
        })}
      </ol>
      <p className="mt-2.5 font-mono text-[11px] uppercase tracking-[0.2em] text-dim">
        <span className="text-[#c79a52]">{String(currentStep).padStart(2, "0")}</span>
        &nbsp;&nbsp;{activeLabel}
      </p>
    </nav>
  );
